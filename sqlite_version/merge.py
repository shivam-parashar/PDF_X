#first write before the loop - init()
#then use - readfile()
#after ending of loop write outside the loop - universal.workbook.close()
...
def init():       #for initializing the xlsx file
  fname = (str)input("Enter year name(without extension) : ")
  universal.workbook = xlsxwriter.Workbook(fname+".xlsx")
...
def readfile():
    wb = load_workbook(universal.filename)
    ws = wb.active
    for row in range(1,universal.worksheet.rows):
        universal.data["Application No."] = ws['A'+str(row)]
        universal.data["Date of filing of Application"] = ws['B'+str(row)]
        universal.data["Publication Date"] = ws['C'+str(row)]
        universal.data["Name of Applicant" = ws['D'+str(row)]
        universal.data["Title of the invention"] = ws['E'+str(row)]
        universal.data["Name of Inventor"] = ws['F'+str(row)]
        universal.data["Abstract"] = ws['G'+str(row)]
        universal.data["No. of Pages"] = ws['H'+str(row)]
        universal.data["No. of Claims"] = ws['I'+str(row)]
        universal.data["International classification"] = ws['J'+str(row)]
        universal.data["Priority Document No"] = ws['K'+str(row)]
        universal.data["Priority Date"] = ws['L'+str(row)]
        universal.data["Name of priority country"] = ws['M'+str(row)]
        universal.data["International Application No"] = ws['N'+str(row)]
        universal.data["IAFiling Date"] = ws['O'+str(row)]
        universal.data["International Publication No"] = ws['P'+str(row)]
        universal.data["Patent of Addition to Application Number"] = ws['Q'+str(row)]
        universal.data["IBFiling Date"] = ws['R'+str(row)]
        universal.data["Divisional to Application Number"] = ws['S'+str(row)]               
        universal.data["ICFiling Date"] = ws['T'+str(row)]
        loop()
   wb.close()    
